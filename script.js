...
async function initFirstRun() {
    try {
        ...
        // FIX: Se il server restituisce un errore globale (es. MPD Offline), bypassiamo l'installazione
        if (data.error) return true;
        
        // Verifica rigorosa per evitare falsi positivi
        if (data.ready === false) {
            ...
            overlay.innerHTML = `
                ...
                <p style="font-size:0.8rem; color:#666;">L'operazione richiede internet. La pagina si ricaricherà da sola a fine processo.</p>
            `;
            ...
            if (installData.success) {
                ...
                alert("Errore pacchettizzazione: " + (installData.error || 'Sconosciuto')); 
                overlay.remove(); 
            }
            return false; 
        }
...
function t(key) {
    ...
    return (typeof translations !== 'undefined' && translations[lng] && translations[lng][key]) ? translations[lng][key] : key;
}
...
function showVolumeOSD(vol, isMute = false) {
    ...
    volumeOsdTimer = setTimeout(() => { osd.classList.remove('show'); }, 2500);
}

function setVolume(val) { 
    sendCmd('volume', val); 
    let localPlayer = document.getElementById('localAudioPlayer');
    if (localPlayer) localPlayer.volume = val / 100;
...
function updateUITheme(style) {
    ...
    let logoText = "CLASSIC '70";
    let fontClass = "blue_mod70";
    ...
    if (style === 'blue_mono_mod1000') { themeClass = 'theme-blue_mono'; logoText = "MONO 1000"; fontClass = "blue_mono"; }
        ...
        themeClass = 'theme-vfd_peak'; 
        logoText = (style === 'vfd_mod85') ? "VFD PEAK '85" : "STUDIO METER"; 
        fontClass = (style === 'vfd_mod85') ? "vfd_peak" : "peppy"; 
        if (style !== 'vfd_mod85') bLogo.style.fontFamily = "sans-serif"; 
    }
    else if (style === 'studio_mod77' || style === 'minimal_mod00') { 
        themeClass = 'theme-studio_master'; 
        logoText = (style === 'studio_mod77') ? "STUDIO MASTER '77" : "STUDIO METER"; 
        fontClass = "peppy"; 
    ...
    try { 
        ...
        let cleanStyle = style.replace('vu_', ''); 
...
async function fetchStatus() {
    ...
    try {
        ...
        if (d.state) {
            ...
            ['btnPlay', 'ssBtnPlay', 'mpBtnPlay'].forEach(id => {
                ...
                if (btn) {
                    ...
                    if (isPlay && !hasPause) btn.innerHTML = '<i class="bi bi-pause-fill"></i>';
                    else if (!isPlay && hasPause) btn.innerHTML = '<i class="bi bi-play-fill"></i>';
                }
            });
        }
        updateNowPlaying(d);
        ...
        if (isStream && (e.name === 'AbortError' || !navigator.onLine)) {
            ...
            if (btnPlay && !btnPlay.innerHTML.includes('play-fill')) btnPlay.innerHTML = '<i class="bi bi-play-fill"></i>';
...
function updateNowPlaying(d) {
    try {
        ...
        let isStream = (fileStr.startsWith('http://') || fileStr.startsWith('https://') || fileUpper.startsWith('RADIO/') || fileUpper.startsWith('WEBRADIO/') || fileUpper.endsWith('.M3U') || fileUpper.endsWith('.PLS') || fileUpper.endsWith('.M3U8'));
        ...
        if (!isFirstLoad) {
            shouldWakeUp = isStream ? fileChanged : (fileChanged || titleChanged);
        }
        ...
        if (shouldWakeUp && wakeEnabled) {
            ...
            if (activeTab && activeTab.id !== 'tab-nowplaying') switchTab('nowplaying');
        }
        ...
        if (isStream) {
            ...
            if (extMatch) { 
                ...
                inferredExtension = ['M3U', 'PLS', 'M3U8'].includes(ext) ? 'WEB STREAM' : ext; 
            } else { 
        ...
        if (isStream) {
            ...
            if (typeof savedRadios !== 'undefined') {
                let matchedRadio = savedRadios.find(r => fileStr.includes(r.url) || r.url.includes(fileStr) || (d.title && r.name && d.title.toLowerCase().includes(r.name.toLowerCase())) || (d.artist && r.name && d.artist.toLowerCase().includes(r.name.toLowerCase())));
                if (matchedRadio && matchedRadio.cover) stationLogo = matchedRadio.cover;
            }
            if (!finalCoverUrl) finalCoverUrl = stationLogo;
        ...
        if (formatStr) {
            ...
            if (bitrate > 0) { 
                if (bitrate >= 1000) sourceDetails += `, ${(bitrate / 1000).toFixed(3)} Mbps`; 
                else sourceDetails += `, ${bitrate} kbps`; 
            }
        }

        let techLine1 = `${codec}`; 
        if (sourceDetails) techLine1 += ` | ${sourceDetails}`;
        ...
        if (ssTrackInfo) {
            ...
            parts.push(codec); 
            if (sourceDetails) parts.push(sourceDetails); 
            ...
            if (isStream && fileStr) parts.push(`URL: ${fileStr}`);
            
            let trackDisplay = parts.join('   -   '); 
            if (ssTrackInfo.textContent !== trackDisplay) ssTrackInfo.textContent = trackDisplay;
        }
...
function toggleView(containerId) {
    ...
    if (icon) icon.className = viewModes[containerId] === 'grid' ? 'bi bi-list' : 'bi bi-grid-fill';
    ...
    if (activeTab) {
        ...
        if (tid === 'folders') loadFolders(currentFolderPath, true); 
            ...
            if (albumState.active) loadAlbumTracks(albumState.album, 'album', true); 
            else loadAlbums(true); 
        } 
        else if (tid === 'artists') { 
            if (artistState.album) loadAlbumTracks(artistState.album, 'artist', true); 
            else if (artistState.artist) loadArtistAlbums(artistState.artist, true); 
            else loadArtists(true); 
        } 
...
function buildItemHtml(mode, dataAttr, safeActionPath, imgUrl, title, subtitle, isDir, iconClass) {
    ...
    if (dataAttr && iconClass.includes('person')) { 
        ...
        if (pathUpper === 'WEBRADIO' || pathUpper === 'RADIO' || pathUpper === 'DEFAULTPLAYLIST') {
            showPlayBtn = false; 
        }
    } else { 
    ...
    if (!finalImgUrl && cacheKey && globalCoverCache[cacheKey]) finalImgUrl = globalCoverCache[cacheKey];
    ...
    let playBtnHtml = showPlayBtn ? 
        `<button class="btn-addplay" style="margin-top:0; flex: 3;" onclick="playAdd('${safeActionPath}', '${pType}', 'play'); event.stopPropagation();"><i class="bi bi-play-fill"></i> Play</button>
         ...
         ${favBtnHtml}` : 
        `<div style="flex:4;"></div>${favBtnHtml}`;
    ...
    if (mode === 'grid') {
        ...
        <div class="grid-item" ${dataTag} data-letter="${sortChar}">
            <div class="grid-cover" onclick="${mainClick}" oncontextmenu="showContextMenu(event, '${safeActionPath}')">
                ${iconHtml}${imgHtml}
            </div>
        ...
        <div class="list-item" ${dataTag} data-letter="${sortChar}">
            <div class="list-cover" onclick="${mainClick}" oncontextmenu="showContextMenu(event, '${safeActionPath}')">
                ${iconHtml}${imgHtml}
            </div>
            <div class="list-item-content" onclick="${mainClick}">
                <div style="font-weight:bold; color:#fff; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${title}</div>
                <div style="font-size:0.8rem; color:#aaa;">${subtitle}</div>
            </div>
    ...
    entries.forEach(entry => {
        if (entry.isIntersecting) {
            ...
            if (img && !img.dataset.loaded) {
                ...
                if (album) { 
                    endpoint = `album_cover&album=${encodeURIComponent(album)}`; 
                    cacheKey = album; 
                } else if (artist) { 
                    endpoint = `artist_cover&artist=${encodeURIComponent(artist)}`; 
                    cacheKey = artist; 
                } else if (folder) { 
...
async function loadSettings() { 
    ...
    try { 
        ...
        if (clientRes.ok && !clientData.error && Object.keys(clientData).length > 0) { 
            ...
            if (globalRes.ok) defaultsToUse = await globalRes.json(); 
        }
        ...
        if (Object.keys(defaultsToUse).length > 0) { 
            ...
            // Novità: Recupera anche lo stato del DSP dal salvataggio se disponibile
            if (defaultsToUse.camillaDspState !== undefined) {
                localStorage.setItem('camilla_dsp_state', defaultsToUse.camillaDspState);
            }
        }
    } catch (e) {} 
    ...
    // --- GESTIONE REALE DEL DSP ALL'AVVIO ---
    let camillaTgl = document.getElementById('camillaDspToggle'); 
    if (camillaTgl) {
        let isDspActive = localStorage.getItem('camilla_dsp_state') === 'true';
        camillaTgl.checked = isDspActive;
        
        let cmd = isDspActive ? 'camilladsp_on' : 'camilladsp_off';
        fetch(`${API}?action=sys_control&cmd=${cmd}`).catch(e => console.error("Errore avvio DSP:", e));
    }
    // ----------------------------------------
...
async function saveClientSettings() {
    let payload = { 
        ...
        // Includiamo lo stato del DSP nel payload
        camillaDspState: document.getElementById('camillaDspToggle') ? document.getElementById('camillaDspToggle').checked : false
    };
...
function saveSettings() {
    ...
    let payload = {
        ssTimeout, ssRandom, vuStyle, wsUri, moodeUrl, 
        streamUrl, backlight, inlineVu, localAnalyzer, deviceLayout
    };
    ...
    let inlineC = document.getElementById('inlineVuContainer'); 
    if (inlineC) inlineC.style.display = inlineVu ? 'flex' : 'none';
}
...
window.onload = async () => {
    let isFirstRunOk = await initFirstRun(); 
    if (!isFirstRunOk) return;
    ...
    fetch(`${API}?action=sys_optimize&mode=startup`).catch(e => console.error(e));
    
    if (typeof initScreensaverAndVU === 'function') initScreensaverAndVU();
...
// Modificato intervallo da 400 a 1000 per salvaguardare la memoria
...
async function sendCmd(cmd, param = '', type = '') { 
    ...
    // QUI C'ERA L'ERRORE: mancava la parentesi tonda dopo l'if
    if (['play', 'next', 'previous', 'playid'].includes(finalCmd)) { 
        ['btnPlay', 'ssBtnPlay', 'mpBtnPlay'].forEach(id => { 
            let btn = document.getElementById(id); 
            if (btn && !btn.innerHTML.includes('pause-fill')) btn.innerHTML = '<i class="bi bi-pause-fill"></i>'; 
        }); 
    } else if (['pause', 'stop'].includes(finalCmd)) { 
        ['btnPlay', 'ssBtnPlay', 'mpBtnPlay'].forEach(id => { 
            let btn = document.getElementById(id); 
            if (btn && btn.innerHTML.includes('pause-fill')) btn.innerHTML = '<i class="bi bi-play-fill"></i>'; 
        }); 
    }
...
async function loadQueue(forceScroll = false) { 
    ...
    data.forEach((s, i) => { 
        ...
        html += `
        <div class="list-item ${s.current ? 'queue-playing' : ''}">
            <div style="width:30px; color:#777;">${i+1}</div>
            <div class="list-item-content" onclick="sendCmd('playid', '${s.id}')">
                <div style="font-weight:bold; color:#fff;">${s.title}</div>
                <div style="font-size:0.8rem; color:#aaa;">${s.artist || s.file}</div>
            </div>
            <div style="color:#777; margin-right: 1rem;">${formatTime(s.duration)}</div>
            <button class="btn-addplay" style="width:auto; margin-top:0;" onclick="sendCmd('playid', '${s.id}')">
...
function toggleLocalStream() {
    ...
    if (!streamUrl) { 
        alert(t('msg_stream_url_missing')); 
        return; 
    }
...
async function loadPlaylists() { 
    ...
    data.forEach(p => { 
        ...
        <div class="list-item" data-letter="${sortChar}">
            <i class="bi bi-journal-album" style="font-size:1.5rem; color:#888; margin-right:15px;"></i>
            <div class="list-item-content" onclick="playPlaylist('${safeP}')">
                <div style="font-weight:bold; font-size:1.1rem; color:#fff;">${p}</div>
            </div>
            <div style="display:flex; gap: 5px;">
                <button class="btn-addplay" style="width:auto; margin-top:0;" onclick="playPlaylist('${safeP}')">
                    <i class="bi bi-play-fill"></i> Play
                </button>
                <button class="btn-addplay" style="width:auto; margin-top:0; border-color:rgba(220,53,69,0.5); color:#dc3545; background:rgba(220,53,69,0.1);" onclick="deletePlaylist('${safeP}')">
    ...
    if (alphaPl) { 
        alphaPl.style.display = 'flex'; 
        resetAlphaFilter('alpha-playlists', 'playlistGrid'); 
    }
}
...
async function loadAlbums(useCache = false) {
    ...
    if (alphaAlbums) { 
        alphaAlbums.style.display = 'flex'; 
        resetAlphaFilter('alpha-albums', 'albumGrid'); 
    }
}
...
async function loadAlbumTracks(album, context = 'album', useCache = false) { 
    ...
    if (context === 'album') { 
        ...
        let bb = document.getElementById('albumBackBtn'); 
        if (bb) bb.style.display = 'flex'; 
        let ap = document.getElementById('albumPath'); 
        if (ap) ap.textContent = album; 
        let aa = document.getElementById('alpha-albums'); 
        if (aa) aa.style.display = 'none'; 
    } else { 
        artistState.album = album; 
        let ap = document.getElementById('artistPath'); 
        if (ap) ap.textContent = `${artistState.artist} / ${album}`; 
        let aa = document.getElementById('alpha-artists'); 
        if (aa) aa.style.display = 'none'; 
    } 
...
function navAlbumsUp() { 
    if (albumState.active) loadAlbums(); 
}
...
async function loadArtists(useCache = false) { 
    ...
    let abb = document.getElementById('artistBackBtn'); 
    if (abb) abb.style.display = 'none'; 
    let ap = document.getElementById('artistPath'); 
    if (ap) ap.textContent = t('tab_artists'); 
    ...
    if (aart) { 
        aart.style.display = 'flex'; 
        resetAlphaFilter('alpha-artists', 'artistGrid'); 
    }
}

async function loadArtistAlbums(artist, useCache = false) { 
    artistState.artist = artist; 
    artistState.album = ''; 
    
    let abb = document.getElementById('artistBackBtn'); 
    if (abb) abb.style.display = 'flex'; 
    let ap = document.getElementById('artistPath'); 
    if (ap) ap.textContent = artist; 
    let aart = document.getElementById('alpha-artists'); 
    if (aart) aart.style.display = 'none';
...
function navArtistsUp() { 
    if (artistState.album) loadArtistAlbums(artistState.artist); 
    else if (artistState.artist) loadArtists(); 
}
                ...
                if (data.length > 0) { 
                    data.forEach(d => { 
                        ...
                        if (rawParam && rawParam.toUpperCase().startsWith('RADIO/')) { 
                            ...
                            subStr = 'Web Radio'; 
                            titleStr = titleStr.replace(/\.(pls|m3u|m3u8)$/i, ''); 
                        } 
...
function switchTab(tabId) {
    ...
    if (tabId === 'queue') loadQueue(!isAlreadyActive); 
...
async function clearRackCache(btn) { 
    if (confirm(t('confirm_sync'))) { 
        ...
        cachedFolders = { path: null, data: [] }; 
        cachedAlbums = null; 
        cachedArtists = null; 
        cachedSubLevel = { id: null, data: [] }; 
        cachedMpdRadios = null; 
        cachedRbRadios = null;
        ...
        let alg = document.getElementById('albumGrid'); 
        if (alg) alg.innerHTML = ''; 
        let artg = document.getElementById('artistGrid'); 
        if (artg) artg.innerHTML = ''; 
        let fog = document.getElementById('folderGrid'); 
...
function applyAlphaFilter(gridId, letter, btn, filterContainerId) {
    ...
    if (gridId === 'radioListGrid') { 
        ...
        headers.forEach(h => { 
            h.style.display = letter === 'ALL' ? 'block' : 'none'; 
        }); 
    }
}
...
function toggleUniversalFav(btn, type, path, title, cover, subtitle, iconClass) {
    ...
    if (index > -1) { 
        universalFavorites.splice(index, 1); 
        isFav = false; 
    } else { 
        universalFavorites.push({ type, path, title, cover, subtitle, iconClass }); 
        isFav = true; 
    }
...
function renderFavorites() {
    ...
    universalFavorites.forEach(f => { 
        if (categories[f.type]) { 
            categories[f.type].items.push(f); 
        } else { 
            categories['file'].items.push(f); 
        } 
    });
...
async function renderRadioList(useCache = false) {
    ...
    if (savedRadios.length > 0) { 
        ...
        savedRadios.forEach((radio, index) => { 
            ...
            let heartIcon = isFav ? 'bi-heart-fill' : 'bi-heart';
            let heartColor = isFav ? 'color: #dc3545;' : 'color: #aaa;';
            let safeName = (radio.name || '').replace(/'/g, "\\'");
            let safeCover = (radio.cover || '').replace(/'/g, "\\'"); 
            ...
            if (mode === 'grid') { 
                ...
                <div class="grid-item" data-letter="${sortChar}">
                    <div class="grid-cover" onclick="playAdd('${safeUrl}', 'file', 'play')">
                        <i class="bi bi-broadcast" ${iconDisplay}></i>
                        <img class="lazy-cover" id="radio-cover-${index}" ${coverDisplay}>
                    </div>
                ...
                html += `
                <div class="list-item" data-letter="${sortChar}">
                    <div class="list-cover" onclick="playAdd('${safeUrl}', 'file', 'play')">
                        <i class="bi bi-broadcast" ${iconDisplay}></i>
                        <img class="lazy-cover" id="radio-cover-${index}" ${coverDisplay}>
                    </div>
                    <div class="list-item-content" onclick="playAdd('${safeUrl}', 'file', 'play')" style="min-width: 0; padding-right: 10px;">
                        <div style="font-weight:bold; color:#fff; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${radio.name}</div>
                        <div style="font-size:0.8rem; color:#aaa; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${radio.url}</div>
                    </div>
    ...
    if (!useCache || cachedMpdRadios === null || cachedRbRadios === null) { 
        ...
        try { 
            let res = await fetch(`${API}?action=radios`); 
            cachedMpdRadios = res.ok ? await res.json() : []; 
        } catch(e) { cachedMpdRadios = []; } 
        
        try { 
            let res2 = await fetch(`${API}?action=radio_recent`); 
            cachedRbRadios = res2.ok ? await res2.json() : []; 
        } catch(e) { cachedRbRadios = []; } 
    }
...
function addCustomRadio() { 
    ...
    savedRadios.push({ 
        ...
        cover: logoInput.value ? logoInput.value.trim() : "" 
    }); 
...
function changeCover(folderPath) {
    ...
    input.onchange = e => {
        ...
        if (file.size > 2 * 1024 * 1024) { 
            alert(`L'immagine è troppo pesante. Il limite è 2MB.`); 
            return; 
        }
        ...
        img.onload = function() {
            ...
            if (this.width > 1200 || this.height > 1200) { 
                alert(`Le dimensioni dell'immagine (${this.width}x${this.height}) superano il limite massimo di 1200x1200 pixel.`); 
                return; 
            }
            ...
            fetch(`${API}?action=set_custom_cover`, { method: 'POST', body: formData })
            .then(res => res.json())
            .then(data => {
                if (data.success) { 
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    const toggle = document.getElementById('miniPlayerSsToggle');
    if (toggle) toggle.checked = preferenza !== 'false'; 
});

function exportConfig() {
    const keysToExport = [
        'hifi_universal_favs', 'hifi_webradios', 'camilla_ss_timeout', 'camilla_ss_random', 
        'camilla_vu_style', 'camilla_ws_uri', 'moode_ui_url', 'camilla_stream_url', 
        'camilla_backlight', 'camilla_inline_vu', 'camilla_local_analyzer', 'camilla_device_layout', 
        'camilla_dsp_state', 'hifi_language', 'view_folderGrid', 'view_albumGrid', 
        'view_artistGrid', 'view_searchGrid', 'view_radioListGrid', 'mostraMiniPlayerSS'
    ];
    let config = {}; 
...
function searchWebCover() {
    ...
    .then(data => {
        if (data.results.length === 0) { 
            resultsDiv.innerHTML = '<div style="color:#dc3545; grid-column:1/-1; text-align:center;">Nessun risultato trovato. Prova a semplificare il testo della ricerca.</div>'; 
            return; 
        }
...
function initKdsiEngine() {
    // Gestito e sovrascritto in salvaschermo.js, 
    // mantenuto qui per evitare errori di dipendenza in assenza dell'altro file.
}

// --- GESTIONE MANUALE DELLA ROTELLA VOLUME ORIZZONTALE ---
document.addEventListener('DOMContentLoaded', () => {
    let volWheel = document.getElementById('volWheelUI');
    let volContainer = document.getElementById('volWheelContainer');
    let volText = document.getElementById('volKnobText');
    let hiddenSlider = document.getElementById('volSlider');
    
    let isDraggingWheel = false;
    let startX = 0;
    let startVol = 0;

    // Sincronizza la grafica della rotella in orizzontale
    function syncWheelVisuals(val, isDragging = false) {
        let v = Math.max(0, Math.min(100, parseFloat(val) || 0));
        
        if (volWheel) {
            volWheel.style.transition = isDragging ? 'none' : 'background-position 0.2s ease-out';
            volWheel.style.backgroundPosition = `${v * 12}px 0px`;
        }
        
        if (volText) volText.textContent = Math.round(v);
    }

    if (hiddenSlider) {
        setInterval(() => {
            if (!isDraggingWheel) syncWheelVisuals(hiddenSlider.value, false);
        }, 500);
    }
    ...
    if (volContainer) {
        volContainer.addEventListener('pointerdown', (e) => {
            ...
            startX = e.clientX; 
            startVol = parseFloat(hiddenSlider.value) || 0;
        ...
        window.addEventListener('pointermove', (e) => {
            ...
            let rawVol = Math.max(0, Math.min(100, startVol + (deltaX * sensitivity)));
            
            syncWheelVisuals(rawVol, true);
            
            let displayVol = Math.round(rawVol);
            showVolumeOSD(displayVol, displayVol === 0);
        });
        ...
        window.addEventListener('pointerup', (e) => {
            if (isDraggingWheel) {
                ...
                let deltaX = e.clientX - startX;
                let sensitivity = 0.3;
                let finalVol = Math.round(Math.max(0, Math.min(100, startVol + (deltaX * sensitivity))));
                
                if (hiddenSlider) hiddenSlider.value = finalVol;
                setVolume(finalVol); 
            }
        });

        // Rotellina del mouse o touchpad
        volContainer.addEventListener('wheel', (e) => {
            e.preventDefault();
            let dir = (e.deltaY < 0 || e.deltaX > 0) ? 2 : -2; 
            let current = parseInt(hiddenSlider.value) || 0;
            let newVol = Math.max(0, Math.min(100, current + dir));
            
            if (hiddenSlider) hiddenSlider.value = newVol;
            syncWheelVisuals(newVol, false);
            setVolume(newVol);
        }, {passive: false});
    }
});
