/**
 * =========================================
 * MOTORE 3D CSS PER VINTAGE RADIOS
 * =========================================
 */
...
// Nomi identificativi dei modelli disponibili
...
/**
 * Recupera l'etichetta personalizzata (legal-safe) dalla cache 
 * per evitare l'utilizzo non autorizzato di loghi reali.
 */
...
/**
 * Inietta dinamicamente il foglio di stile per la renderizzazione 
 * dei materiali 3D (legno, bachelite, vetro) delle radio.
 */
function injectRadioStyles() {
    ...
    /* FILTRO RUMORE (Grana fotografica/materiale) */
    .radio-noise-overlay {
      position: absolute; top: 0; left: 0; width: 100%; height: 100%;
      opacity: 0.14; pointer-events: none; mix-blend-mode: overlay;
    ...
    /* DIMENSIONI E STRUTTURE CHASSIS */
    .radio-body-3d {
      position: relative; padding: 22px; box-sizing: border-box;
      box-shadow: inset 0 3px 0 rgba(255, 255, 255, 0.3), inset 0 15px 25px rgba(255, 255, 255, 0.15), inset 0 -15px 35px rgba(0, 0, 0, 0.9), inset 12px 0 20px rgba(0,0,0,0.4), inset -12px 0 20px rgba(0,0,0,0.4), 0 25px 50px rgba(0, 0, 0, 0.9), 0 2px 4px rgba(0,0,0,0.5);
    ...
    /* CORNICI, VETRI E MANOPOLE */
    .bezel-gold-dark { width: 100%; height: 100%; border-radius: 20px; border: 4px solid #1a0f05; background: #dca33c; padding: 12px; box-sizing: border-box; box-shadow: inset 0 5px 15px rgba(0,0,0,0.6); display: flex; flex-direction: column; justify-content: space-between; }
    .bezel-full-gold { width: 100%; height: 100%; border-radius: 20px; border: 3px solid #8a6c27; background: linear-gradient(135deg, #d4af37 0%, #aa842b 100%); padding: 15px; box-sizing: border-box; box-shadow: inset 0 5px 15px rgba(0,0,0,0.6), 0 2px 5px rgba(255,255,255,0.3); display: flex; gap: 15px; }
    
    .speaker-superla { background: radial-gradient(circle, transparent 20%, #c49a6c 20%, #c49a6c 40%, transparent 40%, transparent 100%), radial-gradient(circle, transparent 20%, #b08558 20%, #b08558 40%, transparent 40%, transparent 100%), #e6c299; background-size: 6px 6px; background-position: 0 0, 3px 3px; }
    .speaker-dots { background: radial-gradient(#111 35%, transparent 35%), #443325; background-size: 8px 8px; }
    .speaker-zone { border: 3px solid #111; box-shadow: inset 0 8px 16px rgba(0,0,0,0.85), 0 1px 1px rgba(255,255,255,0.05); position: relative; overflow: hidden; }

    .glass-dial { position: relative; background: linear-gradient(to bottom, #111 0%, #1f1305 10%, #ff7b00 50%, #3a1e00 100%); border: 2px solid #0a0402; box-shadow: inset 0 0 25px rgba(0, 0, 0, 0.9), 0 1px 1px rgba(255,255,255,0.1); overflow: hidden; }
    .glass-dial::before { content: ''; position: absolute; top: 0; left: -50%; width: 200%; height: 100%; background: linear-gradient(45deg, transparent 45%, rgba(255,255,255,0.08) 50%, transparent 55%); pointer-events: none; z-index: 3; }
    .brass-bezel { border: 4px solid #d4af37 !important; border-top-color: #fdf0a6 !important; border-bottom-color: #7a5c13 !important; border-left-color: #cca44d !important; border-right-color: #b58c38 !important; box-shadow: 0 1px 2px rgba(0,0,0,0.5), inset 0 4px 10px rgba(0,0,0,0.9) !important; }

    .scale-container { width: 100%; height: 100%; padding: 8px 15px; box-sizing: border-box; display: flex; flex-direction: column; justify-content: space-between; z-index: 1; position: relative; }
    ...
    .scale-lines { flex-grow: 1; height: 7px; background: repeating-linear-gradient(90deg, rgba(255,230,180,0.8) 0px, rgba(255,230,180,0.8) 1px, transparent 1px, transparent 15px); border-bottom: 1px solid rgba(255,230,180,0.5); }
    ...
    .radio-needle { position: absolute; left: 50%; top: 2px; bottom: 2px; width: 2px; background: #e62200; box-shadow: 3px 2px 4px rgba(0, 0, 0, 0.8); z-index: 2; transition: left 0.1s linear; }

    .heavy-knob { width: 36px; height: 36px; border-radius: 50%; border: 2px solid #000; position: relative; box-shadow: 0 4px 6px rgba(0,0,0,0.9), inset 0 1px 2px rgba(255,255,255,0.2); }
    .knob-dark { background: radial-gradient(circle at 30% 30%, #3a1a10 0%, var(--bachelite) 65%); }
    .knob-white { background: radial-gradient(circle at 30% 30%, #fdf7e3 0%, #d8ccb1 65%); border: 1px solid #8c816a; }
    .heavy-knob::before { content: ''; position: absolute; top: 50%; left: 50%; width: 22px; height: 22px; margin: -11px 0 0 -11px; border-radius: 50%; border: 1px dashed rgba(255,255,255,0.15); }
    .heavy-knob::after { content: ''; position: absolute; top: 3px; left: 50%; width: 2px; height: 7px; background: var(--oro-vintage); transform: translateX(-50%); box-shadow: 0 0 2px #000; }
    
    .radio-foot { position: absolute; bottom: -12px; width: 55px; height: 14px; background: linear-gradient(to right, #111, #222 50%, #050505); border-radius: 0 0 12px 12px; box-shadow: 0 12px 15px rgba(0,0,0,0.7); }
...
/**
 * Genera il markup HTML specifico per il modello di radio selezionato.
 * Applica i loghi custom richiamando `getRadioCustomLabel()`.
 */
...
/**
 * Inizializza il DOM inserendo l'HTML della radio nel contenitore padre.
 */
...
/**
 * Smonta e rimuove il componente 3D dalla memoria.
 */
...
/**
 * Ricalcola la scala del modello in base allo spazio disponibile (Viewport).
 */
...
/**
 * Loop di animazione: muove la lancetta e fa lampeggiare i bulbi interni.
 */
